...
To run, execute python serial_document.py at the prompt.
Make sure the dictionary "big.txt" is in the current working directory.
Make sure the document to be corrected "test.txt" is in the current
working directory.
...
if __name__ == "__main__":
    ...
    start_time = time.time()
    correct_document("test.txt", False)
    run_time = time.time() - start_time
    print '-----'
    print '%.2f seconds to run' % run_time
    print '-----'
